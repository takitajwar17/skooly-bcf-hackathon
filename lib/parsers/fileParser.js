...
// Special prefix to identify file URLs stored as content
export const FILE_URL_PREFIX = "FILE_URL:";

/**
 * Extracts text content from various file types
 * @param {string} filePath - Path to the file on disk
 * @param {string} mimeType - MIME type of the file
 * @param {string} cloudinaryUrl - Optional Cloudinary URL for fallback
 * @returns {Promise<{content: string, isFileUrl: boolean}>} Extracted text content or file URL
 */
export const parseFile = async (filePath, mimeType, cloudinaryUrl = null) => {
  try {
    if (mimeType === "application/pdf") {
      try {
        ...
        return { content: data.text, isFileUrl: false };
      } catch (pdfError) {
        console.warn("PDF parsing failed, using Cloudinary URL:", pdfError.message);
        // Fallback: return the Cloudinary URL so Gemini can read the file directly
        if (cloudinaryUrl) {
          return { content: `${FILE_URL_PREFIX}${cloudinaryUrl}`, isFileUrl: true };
        }
        return { content: "", isFileUrl: false };
      }
    } 
    ...
    if (
      ...
      return { content: data.value, isFileUrl: false };
    }
    ...
    // If parsing fails and we have a Cloudinary URL, use it
    if (cloudinaryUrl) {
      return { content: `${FILE_URL_PREFIX}${cloudinaryUrl}`, isFileUrl: true };
    }
    return { content: "", isFileUrl: false };
...
/**
 * Check if content is a file URL reference
 * @param {string} content - Content string to check
 * @returns {boolean} - True if content is a file URL
 */
export const isFileUrlContent = (content) => {
  return content && typeof content === 'string' && content.startsWith(FILE_URL_PREFIX);
};

/**
 * Extract file URL from content
 * @param {string} content - Content string with FILE_URL prefix
 * @returns {string|null} - The file URL or null
 */
export const extractFileUrl = (content) => {
  if (isFileUrlContent(content)) {
    return content.slice(FILE_URL_PREFIX.length);
  }
  return null;
};
