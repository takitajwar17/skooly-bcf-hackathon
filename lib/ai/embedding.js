import { GoogleGenAI } from "@google/genai";

// Initialize the Gen AI client
const ai = new GoogleGenAI({ apiKey: process.env.GOOGLE_API_KEY });

/**
 * Generate text embedding using Gemini (GOOGLE_API_KEY).
 * Uses text-embedding-004 with 768 dimensions to match MongoDB embedding_index.
 */
export async function generateEmbedding(text) {
  const response = await ai.models.embedContent({
    model: "text-embedding-004",
    contents: text,
    config: {
      taskType: "RETRIEVAL_DOCUMENT",
      outputDimensionality: 768,
    },
  });
  return response.embeddings[0].values;
}

/**
 * Generate embedding for a search query (768-dim, matches index).
 */
export async function generateQueryEmbedding(query) {
  const response = await ai.models.embedContent({
    model: "text-embedding-004",
    contents: query,
    config: {
      taskType: "RETRIEVAL_QUERY",
      outputDimensionality: 768,
    },
  });
  return response.embeddings[0].values;
}

/**
 * Fetch file as base64 for Gemini
 * @param {string} url - File URL
 * @returns {Promise<{data: string, mimeType: string}>} - Base64 data and mime type
 */
async function fetchFileAsBase64(url) {
  const response = await fetch(url);
  const buffer = await response.arrayBuffer();
  const base64 = Buffer.from(buffer).toString('base64');
  
  // Determine mime type from URL or response
  const contentType = response.headers.get('content-type') || 'application/pdf';
  
  return {
    data: base64,
    mimeType: contentType.split(';')[0], // Remove charset if present
  };
}

/**
 * Generate chat response with context and optional file URLs
 * Supports RAG with both text context and file URLs (Gemini reads files directly)
 * @param {string} prompt - User prompt
 * @param {string} context - RAG context from materials (text)
 * @param {Array} history - Previous conversation messages
 * @param {Array} fileUrls - Array of file URL objects {url, title, type}
 * @returns {Promise<string>} - Generated response
 */
export async function generateResponse(prompt, context = "", history = [], fileUrls = []) {
  const systemPrompt = `You are Skooly, an AI learning assistant for university courses. 
You help students understand course materials, answer questions, and generate learning content.
Always be helpful, accurate, and cite sources when using provided context.

${context ? `CONTEXT FROM COURSE MATERIALS:\n${context}\n\n` : ""}
${fileUrls.length > 0 ? `I'm also providing you with ${fileUrls.length} document file(s) as additional context. Please analyze their content to answer the question.\n\n` : ""}
Use the context above to answer the student's question. If the answer isn't in the context, say so.`;

  // Build parts for the user message including files
  const userParts = [{ text: systemPrompt }];
  
  // Add file parts if we have file URLs
  for (const fileInfo of fileUrls) {
    try {
      console.log(`Fetching file for RAG: ${fileInfo.title} (${fileInfo.url})`);
      const { data, mimeType } = await fetchFileAsBase64(fileInfo.url);
      userParts.push({
        inlineData: {
          data,
          mimeType,
        }
      });
      console.log(`✓ Added file to context: ${fileInfo.title}`);
    } catch (error) {
      console.error(`Failed to fetch file ${fileInfo.url}:`, error);
    }
  }

  const contents = [
    { role: "user", parts: userParts },
    { role: "model", parts: [{ text: "I understand. I'm Skooly, ready to help with course materials." }] },
    ...history.map((msg) => ({
      role: msg.role === "assistant" ? "model" : "user",
      parts: [{ text: msg.content }],
    })),
    { role: "user", parts: [{ text: prompt }] },
  ];

  const response = await ai.models.generateContent({
    model: "gemini-2.0-flash",
    contents,
  });

  return response.text;
}

/**
 * Generate a direct AI response with no RAG/grounding.
 * Used for community bot replies — Gemini answers from general knowledge only.
 * @param {string} prompt - User question or message
 * @returns {Promise<string>} - Generated response
 */
export async function generateDirectResponse(prompt) {
  const systemPrompt = `You are Skooly, a helpful AI tutor for university students. Answer the student's question clearly and concisely. Use your general knowledge. Be friendly and direct. Format your reply in **Markdown**: use **bold** for emphasis, bullet lists with * for key points, and line breaks between paragraphs.`;

  const contents = [
    { role: "user", parts: [{ text: systemPrompt }] },
    { role: "model", parts: [{ text: "I'm Skooly. I'll help with your question." }] },
    { role: "user", parts: [{ text: prompt }] },
  ];

  const response = await ai.models.generateContent({
    model: "gemini-2.0-flash",
    contents,
  });

  return response.text;
}

/**
 * Generate streaming chat response
 * @param {string} prompt - User prompt
 * @param {string} context - RAG context
 * @param {Array} history - Conversation history
 * @returns {AsyncGenerator} - Streaming response
 */
export async function* generateStreamingResponse(prompt, context = "", history = []) {
  const systemPrompt = `You are Skooly, an AI learning assistant for university courses.
Always be helpful, accurate, and cite sources when using provided context.

${context ? `CONTEXT FROM COURSE MATERIALS:\n${context}\n\n` : ""}`;

  const contents = [
    { role: "user", parts: [{ text: systemPrompt }] },
    { role: "model", parts: [{ text: "Ready to help!" }] },
    ...history.map((msg) => ({
      role: msg.role === "assistant" ? "model" : "user",
      parts: [{ text: msg.content }],
    })),
    { role: "user", parts: [{ text: prompt }] },
  ];

  const response = await ai.models.generateContentStream({
    model: "gemini-2.0-flash",
    contents,
  });

  for await (const chunk of response) {
    if (chunk.text) {
      yield chunk.text;
    }
  }
}

/**
 * Generate theory content (notes, summaries)
 * @param {string} topic - Topic to generate content for
 * @param {string} context - RAG context from materials
 * @returns {Promise<string>} - Generated markdown content
 */
export async function generateTheoryContent(topic, context = "") {
  const prompt = `Generate comprehensive study notes on: "${topic}"

${context ? `Use this course material as reference:\n${context}\n\n` : ""}

Format the notes with:
- Clear headings and subheadings
- Bullet points for key concepts
- Examples where applicable
- Summary at the end

Output in clean Markdown format.`;

  const response = await ai.models.generateContent({
    model: "gemini-2.0-flash",
    contents: prompt,
  });

  return response.text;
}

/**
 * Generate lab/code content
 * @param {string} topic - Programming topic
 * @param {string} language - Programming language
 * @param {string} context - RAG context
 * @returns {Promise<string>} - Generated code with explanation
 */
export async function generateLabContent(topic, language = "python", context = "") {
  const prompt = `Generate a complete code example for: "${topic}" in ${language}

${context ? `Reference material:\n${context}\n\n` : ""}

Include:
1. Brief explanation of the concept
2. Complete, working code with comments
3. Example usage/output
4. Common pitfalls to avoid

Output in Markdown with proper code blocks.`;

  const response = await ai.models.generateContent({
    model: "gemini-2.0-flash",
    contents: prompt,
  });

  return response.text;
}

export default ai;
